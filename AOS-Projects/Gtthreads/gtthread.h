...
typedef enum {EINVAL = 1, ESRCH, EDEADLK, EAGAIN, ENOMEM, EPERM, EBUSY, PTHREAD_CANCELED, SYS_ERR} ERRORS;

typedef unsigned long int gtthread_t;
typedef struct gtthread_attr {
        int valid;
        void  *stack;
	ucontext_t uc;
        void *(*func)(void *);
        ...
        void* ret;
	void **status;
	/* thread which I am waiting for */
        gtthread_t wait;
	/* thread that is waiting for this */
        gtthread_t join;
        int zombie;
	int cancel;
} gtthread_attr_t;
